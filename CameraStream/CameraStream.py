...
DEFAULT_STREAM_PORTS = [
    {
        "Cam_UUID": "usb-H264_USB_Camera_H264_USB_Camera_2020032801-video-index0",
        "Port": 5000
    },
    {
        "Cam_UUID": "usb-HD_USB_Camera_HD_USB_Camera-video-index0",
        "Port": 5001
    }
]
...
class CameraStream:
    ...
    def _host_stream(self, CameraID : str, port : int) -> None:
        ...
        while self._run_threads:
            ...
            if camera: # If the camera with ID exists
                ...
                self.client.containers.run(image=self._image.id,
                                           detach=True,
                                           devices=[camera+":video0:rwm"],
                                           ports={'5000/tcp': port})
                input("Wait a minute")
            else:
                pass
    ...
    def startup(self) -> None:
        ...
        for stream in self.stream_ports:
            thread = threading.Thread(target=self._host_stream, args=(self.image, stream["Cam_UUID"], stream["Port"]))
            self.host_threads += thread
            thread.start()
    ...
    def get_cameras(self) -> list:
        ...
        Example:
            [
                ['/dev/v4l/by-id/usb-H264_USB_Camera_H264_USB_Camera_2020032801-video-index0', '../../video0'],
                ['/dev/v4l/by-id/usb-H264_USB_Camera_H264_USB_Camera_2020032801-video-index1', '../../video1']
            ]
        """
        ...
        return [cam.split(' ') for cam in cameras]
